...
KMSDRM_LEGACY_FBFromBO(_THIS, struct gbm_bo *bo)
{
    ...
    format = KMSDRM_LEGACY_gbm_bo_get_format(bo);
    strides[0] = KMSDRM_LEGACY_gbm_bo_get_stride(bo);
    handles[0] = KMSDRM_LEGACY_gbm_bo_get_handle(bo).u32;
    ret = KMSDRM_LEGACY_drmModeAddFB2(viddata->drm_fd, w, h, format, handles, strides, offsets,
                                  &fb_info->fb_id, 0);
    if (ret) {
      SDL_free(fb_info);
      return NULL;
...
#if 1
    /* IT PVR SGX 530 Driver for BeagleBone Black currently only supports GBM pixel format RGB565. */
